...
const moveGuide=()=>{const guide=document.querySelector('.tv-guide'),layout=document.querySelector('.watch-layout'),panel=document.querySelector('.channel-panel');if(!guide||!layout)return;if(mq.matches){if(guide.parentElement!==layout)layout.appendChild(guide);else if(panel&&guide.previousElementSibling!==panel)layout.appendChild(guide);}else if(guide.parentElement!==document.body)document.body.appendChild(guide)};
